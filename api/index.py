...
def setup_cookies(custom_cookie_str=None):
    ...
    Automatically handles escaped newlines from Vercel environment variables.
    """
    ...
    if cookie_content:
        # Fix unescaped newlines commonly caused by Vercel env variable inputs
...
def get_ydl_options(cookie_path=None, quality='best', client_preset='ios'):
    """
    Generates yt-dlp configurations tailored for bypassing bot detection
    and preventing serverless execution timeouts.
    """
    ...
    # Alternate player client presets to bypass YouTube bot detection
    client_presets = {
        'ios': ['ios', 'mweb'],
        'android': ['android', 'mweb'],
        'tv': ['tv_embedded', 'mweb'],
        'mweb': ['mweb']
    }

    selected_clients = client_presets.get(client_preset, ['ios', 'mweb'])
    ...
    ydl_opts = {
        ...
        'logtostderr': False,
        'geo_bypass': True,
        'socket_timeout': 6,  # Timeout to prevent Vercel 10s serverless kill
        'retries': 1,
        'http_headers': {
...
def fetch_video():
    try:
        ...
        # Multi-tiered extraction attempt across different client signatures
        client_attempts = ['ios', 'android', 'tv']
        
        for client in client_attempts:
            try:
                ydl_opts = get_ydl_options(cookie_path, quality, client_preset=client)
        ...
        if not info:
            raise last_exception or Exception("Failed to extract video information.")
        ...
        if "Sign in to confirm you're not a bot" in error_msg:
            error_msg = "YouTube Bot Block detected. Please use the 'Bypass Bot Fix' button to add cookies, or set the YT_COOKIES environment variable in Vercel."
        elif "403" in error_msg or "Forbidden" in error_msg:
            error_msg = "Provider returned 403 Forbidden. The video may be geo-restricted or blocked on cloud server IPs."
        elif "timed out" in error_msg.lower():
            error_msg = "Extraction timed out. Try switching to Fast/SD quality."
...
<body>
  <div class="card">
    ...
    <p style="color:#94a3b8;font-size:14px;">Your download for <strong>{safe_filename}</strong> will start automatically.</p>
